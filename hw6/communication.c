...
void chat(int sock, char *uname){
	fd_set input;
	char msg[MSG_MAX];
	char userlist[UNAMELIST_SIZE];

	/* give server client's username */ 
	send_uname(sock, uname);

	/* receieve userlist upon logging in */ 
	recv_unamelist(sock, userlist);
...
void send_uname(int sock, char *uname){
	printf("sending uname %s...\n", uname);
	if(write(sock, uname, UNAME_SIZE) == -1){
		fprintf(stderr, "%s: Failed to send username\n", 
				PROGRAM_NAME);
		exit(EXIT_FAILURE);
	}
}
...
void recv_unamelist(int sock, char *userlist){
	if(read(sock, userlist, UNAMELIST_SIZE) < 0){
		fprintf(stderr, "%s: Failed to recieve userlist\n",
				PROGRAM_NAME);
		exit(EXIT_FAILURE);
	}
}
